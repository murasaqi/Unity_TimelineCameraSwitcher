...
using UnityEngine;
     ...
     public static class UnityVectorExtensions_Clone
    {
            ...
            this Quaternion orient, Vector3 lookAtDir, Vector3 worldUp)
        {
            ...
            float angleH = 0;
            {
                ...
                if (!targetDirH.AlmostZero())
                {
                    ...
                    angleH = UnityVectorExtensions_Clone.SignedAngle(currentDirH, targetDirH, up);
                }
            }
            Quaternion q = Quaternion.AngleAxis(angleH, up);

            // Get local vertical angle
            float angleV = UnityVectorExtensions_Clone.SignedAngle(
